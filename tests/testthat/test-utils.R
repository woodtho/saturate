...
test_that("qc_cb_palette cycles when n exceeds palette length", {
  pal <- qc_cb_palette(10L, type = "okabe_ito")
...
test_that("qc_set_code_key sets the code_key field", {
...
test_that("qc_undeprecate_code restores code to active", {
  proj <- make_test_project()
  on.exit(qc_close(proj))
...
# ── qc_update_coding_memo / qc_update_coding_confidence ──────────────────────

test_that("qc_update_coding_memo changes memo", {
  proj <- make_test_project()
  on.exit(qc_close(proj))
...
test_that("qc_update_coding_confidence changes confidence", {
  proj <- make_test_project()
  on.exit(qc_close(proj))
  doc  <- qc_import_document(proj, content = "text here", name = "d")
  c1   <- qc_add_code(proj, "c1")
...
test_that("qc_merge_codings spans from first to last", {
  proj <- make_test_project()
  on.exit(qc_close(proj))
...
test_that("qc_split_coding creates two codings from one", {
  proj <- make_test_project()
  on.exit(qc_close(proj))
  doc  <- qc_import_document(proj, content = "ABCDEFGHIJ", name = "d")
...
test_that("qc_set_source_type updates the source_type field", {
  proj <- make_test_project()
  on.exit(qc_close(proj))
